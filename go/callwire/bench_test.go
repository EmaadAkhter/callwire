...
func BenchmarkLatencyGoToGo(b *testing.B) {
	...
	benches := []struct {
		...
		{"noop", func() { _, _ = Import[int](client, context.Background(), "noop", nil) }},
		{"echo-int", func() { _, _ = Import[int](client, context.Background(), "echo", []interface{}{42}) }},
		...
		{"add", func() { _, _ = Import[int](client, context.Background(), "add", []interface{}{10, 20}) }},
		{"error-path", func() { _, _ = Import[int](client, context.Background(), "errFn", nil) }},
	}
...
`
	...
	benches := []struct {
		...
		{"add", func() { _, _ = Import[int](client, context.Background(), "add", []interface{}{10, 20}) }},
		{"error", func() { _, _ = Import[int](client, context.Background(), "raise_error", nil) }},
	}
...
func BenchmarkLatencyDistribution(b *testing.B) {
	...
	b.Run("echo-int", func(b *testing.B) {
		...
		for i := 0; i < b.N; i++ {
			t0 := time.Now()
			Import[int](client, context.Background(), "echo", []interface{}{42})
		...
		b.Run(fmt.Sprintf("workers-%d", n), func(b *testing.B) {
			...
			for w := 0; w < n; w++ {
				...
				go func() {
					defer wg.Done()
					for range work {
						Import[int](client, context.Background(), "noop", nil)
					}
				}()
			}
			for i := 0; i < b.N; i++ {
...
func BenchmarkSharedConnVsMultiConn(b *testing.B) {
	...
	b.Run("shared-conn", func(b *testing.B) {
		...
		b.RunParallel(func(pb *testing.PB) {
			for pb.Next() {
				Import[int](c, context.Background(), "noop", nil)
			}
		})
	})
	...
	b.Run("with-slow-call", func(b *testing.B) {
		...
		b.RunParallel(func(pb *testing.PB) {
			for pb.Next() {
				...
				if slow {
					Import[int](client, context.Background(), "slow", []interface{}{10})
				} else {
					Import[int](client, context.Background(), "noop", nil)
				}
			}
...
func BenchmarkFuncComplexity(b *testing.B) {
	...
	benches := []struct {
		...
		{"noop", func() { _, _ = Import[int](client, context.Background(), "noop", nil) }},
		{"fib-20", func() { _, _ = Import[int](client, context.Background(), "fib20", nil) }},
		{"fib-30", func() { _, _ = Import[int](client, context.Background(), "fib30", nil) }},
		{"json-marshal", func() { _, _ = Import[string](client, context.Background(), "jsonMarshal", nil) }},
		{"sleep-1ms", func() { _, _ = Import[int](client, context.Background(), "sleep1ms", nil) }},
	}
	for _, bb := range benches {
		b.Run(bb.name, func(b *testing.B) {
			b.ReportAllocs()
...
func BenchmarkArgCount(b *testing.B) {
	...
	for _, n := range counts {
		b.Run(fmt.Sprintf("%d-args", n), func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				Import[int](client, context.Background(), "manyArgs", args)
			}
		})
	}
}
...
func BenchmarkResultSize(b *testing.B) {
	...
	for _, n := range sizes {
		b.Run(fmt.Sprintf("%dB", n), func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				Import[string](client, context.Background(), "bigResult", []interface{}{n})
			}
			b.StopTimer()
...
func BenchmarkStructRoundTrip(b *testing.B) {
	...
	b.Run("10-field", func(b *testing.B) {
		...
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			Import[testStruct10](client, context.Background(), "echo10", []interface{}{s10})
		}
	})
	...
	b.Run("50-field", func(b *testing.B) {
		...
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			Import[testStruct50](client, context.Background(), "echo50", []interface{}{s50})
		}
	})
}
...
func BenchmarkStringSize(b *testing.B) {
	...
	for _, n := range sizes {
		b.Run(fmt.Sprintf("%dB", n), func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				Import[string](client, context.Background(), "echoStr", []interface{}{payload})
			}
			b.StopTimer()
...
func BenchmarkNestedData(b *testing.B) {
	...
	for _, d := range depths {
		b.Run(fmt.Sprintf("depth-%d", d), func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				Import[map[string]interface{}](client, context.Background(), "nestedMap", []interface{}{d})
			}
		})
	}
}
...
func BenchmarkRefWrapper(b *testing.B) {
	...
	b.Run("Import", func(b *testing.B) {
		b.ReportAllocs()
		for i := 0; i < b.N; i++ {
			Import[int](client, context.Background(), "add", []interface{}{3, 4})
		}
	})
...
func BenchmarkGoroutineCount(b *testing.B) {
	...
	for _, n := range []int{1, 10, 100} {
		b.Run(fmt.Sprintf("%d-conns", n), func(b *testing.B) {
			...
			b.ResetTimer()
			for i := 0; i < b.N; i++ {
				Import[int](conns[i%n], context.Background(), "noop", nil)
			}
			b.StopTimer()
			for _, c := range conns {
				c.Close()
...
func BenchmarkPythonVsGoReflection(b *testing.B) {
	...
	b.Run("fib-10", func(b *testing.B) {
		...
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			Import[int](client, context.Background(), "fib20", nil)
		}
	})
	b.Run("fib-30", func(b *testing.B) {
		...
		b.ResetTimer()
		for i := 0; i < b.N; i++ {
			Import[int](client, context.Background(), "fib30", nil)
		}
	})
}
